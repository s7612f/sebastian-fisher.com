...
function calcMacros() {
  ...
  var heightV = parseFloat(document.getElementById('c-height').value);
  ...
  var out     = document.getElementById('macro-result');

  if (!weight || !heightV || !age) {
    out.style.display = 'block';
    out.innerHTML = '<span style="color:var(--accent);">Fill in all fields.</span>';
    return;
  }
  ...
  var bmr;
  if (sex === 'male') {
    bmr = 10 * weight + 6.25 * heightV - 5 * age + 5;
  } else {
    bmr = 10 * weight + 6.25 * heightV - 5 * age - 161;
  }
  ...
  else if (goal === 'bulk')      kcal = tdee + 400;
  else if (goal === 'agbulk')    kcal = tdee + 700;
  else                            kcal = tdee;

  var protein = Math.round(weight * 2.2);
  var fat     = Math.round(weight * 1.0);
  var fatKcal = fat * 9;
  var proKcal = protein * 4;
  var carbKcal= Math.round(kcal) - proKcal - fatKcal;
  var carbs   = Math.round(carbKcal / 4);

  var goalLabel = { cut: 'CUT', maintain: 'MAINTAIN', bulk: 'LEAN BULK', agbulk: 'AGGRESSIVE BULK' }[goal] || goal.toUpperCase();

  out.style.display = 'block';
  out.innerHTML =
    '<strong>GOAL: ' + goalLabel + '</strong><br>' +
    'Estimated BMR: ' + Math.round(bmr) + ' kcal<br>' +
    'TDEE (maintenance): ' + Math.round(tdee) + ' kcal<br>' +
    '<br>' +
    '<strong>TARGET DAILY INTAKE</strong><br>' +
    'Calories: ' + Math.round(kcal) + ' kcal<br>' +
    'Protein:  ' + protein + 'g (' + proKcal + ' kcal)<br>' +
    'Fat:      ' + fat + 'g (' + fatKcal + ' kcal)<br>' +
    'Carbs:    ' + (carbs > 0 ? carbs : 0) + 'g (' + Math.max(0, carbKcal) + ' kcal)<br>' +
    '<br>' +
    '<span style="color:var(--fg-dim);font-size:0.82em;">Numbers are estimates. Adjust based on weekly weight trend.</span>';
}
...
function calcORM() {
  ...
  var out    = document.getElementById('orm-result');

  if (!weight || !reps || reps < 1) {
    out.style.display = 'block';
    out.innerHTML = '<span style="color:var(--accent);">Enter weight and reps.</span>';
    return;
  }

  if (reps === 1) {
    out.style.display = 'block';
    out.innerHTML = 'Estimated 1RM: <strong>' + weight + ' kg</strong> (that was your 1RM)';
    return;
  }
  ...
  var rows = pcts.map(function (p) {
    return '<tr><td>' + p + '%</td><td>' + Math.round(orm * p / 100) + ' kg</td></tr>';
  }).join('');

  out.style.display = 'block';
  out.innerHTML =
    'Estimated 1RM: <strong>' + Math.round(orm) + ' kg</strong><br><br>' +
    '<table class="retro-table"><tr><th>%</th><th>Weight</th></tr>' + rows + '</table>';
}

function calcTimeline() {
  var current = parseFloat(document.getElementById('tl-current').value);
  var target  = parseFloat(document.getElementById('tl-target').value);
  var rate    = parseFloat(document.getElementById('tl-rate').value);
  var out     = document.getElementById('tl-result');
  ...
  var dir   = diff > 0 ? 'gain' : 'lose';

  out.style.display = 'block';
  out.innerHTML =
    'To ' + dir + ' ' + Math.abs(diff).toFixed(1) + ' kg at ' + Math.abs(rate) + ' kg/week:<br>' +
    '<strong>' + Math.round(weeks) + ' weeks</strong> (~' + months.toFixed(1) + ' months)<br>' +
    '<span style="color:var(--fg-dim);font-size:0.82em;">Actual rate varies. Recheck every 2 weeks and adjust calories.</span>';
}
